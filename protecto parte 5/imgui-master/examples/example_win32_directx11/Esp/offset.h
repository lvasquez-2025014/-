...
// Inicializar offsets seg�n el juego
void initOffsets(int game)
{
    if (game == 0) {
        ...
        // Offsets de Huesos (ltima actualizacin)
        Head = 0x45C;            // Cabeza
        Neck = 0x460;            // Cuello
        ...
        // FF MAX 
        GamePackage = "com.dts.freefiremax";
        ...
        ViewMatrix = 0xe8;
        Head = 0x45c;          // Đầu
        Neck = 0x464;          // Cổ (Mismo que Spine en MAX)
        Spine = 0x464;         // Xương sống / ngực / cổ
        Hip = 0x460;           // Hông / chậu
        Root = 0x470;          // Gốc nhân vật / tâm model

        // ===== Left Arm / Tay trái =====
        LeftShoulder = 0x490;  // Vai trái
        LeftSholder = LeftShoulder;
        LeftElbow = 0x4A0;     // Khuỷu tay trái
        LeftWristJoint = 0x49c;// Khớp cổ tay trái
        LeftWrist = LeftWristJoint;
        LeftHand = 0x488;      // Bàn tay trái

        // ===== Right Arm / Tay phải =====
        RightShoulder = 0x494; // Vai phải
        RightSholder = RightShoulder;
        RightElbow = 0x4A4;    // Khuỷu tay phải
        RightWristJoint = 0x498;// Khớp cổ tay phải
        RightWrist = RightWristJoint;
        RightHand = 0x484;     // Bàn tay phải

        // ===== Left Leg / Chân trái =====
        LeftCalf = 0x478;      // Bắp chân trái / đầu gối dưới
        LeftFoot = 0x480;      // Bàn chân trái

        // ===== Right Leg / Chân phải =====
        RightCalf = 0x468;     // Bắp chân phải / đầu gối dưới
        RightFoot = 0x47c;
    }
    else if (game == 2) {
        // FF MAX Beta
        ...
        // ===== Right Arm / Tay phải =====
        RightShoulder = 0x494; // Vai java / Vai phải
        RightSholder = RightShoulder;
        RightElbow = 0x4A4;    // Khuỷu tay phải
        RightWristJoint = 0x498;// Khớp cổ tay phải
        RightWrist = RightWristJoint;
