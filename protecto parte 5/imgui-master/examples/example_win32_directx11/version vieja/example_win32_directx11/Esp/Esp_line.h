...
struct DefaultInitializer {
    DefaultInitializer() {
        g_EspEnabled = true;   // FIX: habilitar ESP por defecto
        g_EspBox = false;
        g_EspBoxStyle = 1;     // FIX: caja normal por defecto
        g_EspName = true;      // FIX: mostrar nombre por defecto
        g_EspHealth = false;
        g_EspShield = false;
        g_EspDistance = true;  // FIX: mostrar distancia por defecto
        g_EspID = false;
        g_EspBones = false;
        g_EspWeaponIcon = false;
        g_EspWeaponText = false;
        g_EspKnockedIcon = false;
        g_EspLine = true;      // FIX: habilitar linea ESP por defecto
        g_EspLineStyle = 0;
        g_EspLineRainbow = false;
        g_EspRainbowGlobal = false;
        g_EspLineGlow = false;
        g_EspLineThickness = 1.5f;
        g_EspMaxDistance = 0.0f;
        g_EspNearestOnly = false;
        g_EspShowTeam = false;
        g_EspShowInvisible = true; // FIX: mostrar invisibles por defecto
...
static bool GetGameData(uint32_t& localPlayer, Matrix4x4& viewMatrix, Vector3& localPos, uint32_t& entities, int& entitiesCount, int& emuWidth, int& emuHeight, int& clientX, int& clientY)
{
    ...
    if (emuWidth <= 0 || emuHeight <= 0) return false;
    ...
    // FIX: Verificar MatchStatus para asegurar que hay una partida activa (igual que version basica)
    uint32_t matchStatus = 0;
    if (!ReadZ(currentMatch + MatchStatus, matchStatus) || matchStatus == 0) return false;
...
inline uint32_t zGetInfoW(uint32_t player, bool zWOnScan = true)
{
    uint32_t zWdata = 0;
    if (ReadZ(player + Weapon, zWdata) && zWdata)
    {
        uint32_t zWdata1 = 0;
        if (ReadZ(zWdata + 0x8, zWdata1) && zWdata1)
        {
            uint32_t zWdata2 = 0;
            if (ReadZ(zWdata1 + 0x10, zWdata2) && zWdata2)
            {
                uint32_t zWID = 0;
...
static TargetInfo ReadEnemyData(uint32_t entity, const Vector3& localPos, const Matrix4x4& viewMatrix, int emuWidth, int emuHeight)
{
    ...
    if (g_EspBones || g_AimAssist) {
        uint32_t bones[14] = { Head, Neck, Spine, Hip, LeftSholder, LeftElbow, LeftHand, RightSholder, RightElbow, RightHand, LeftCalf, LeftFoot, RightCalf, RightFoot };
...
void ESP_line()
{
    ...
    // FIX: No retornar si g_EspEnabled esta activo aunque los demas flags esten apagados
    if (!anyEspActive && !g_EspEnabled) return;
    ...
    if (dataValid) {
        for (uint32_t i = 0; i < entitiesCount; i++) {
            ...
            // FIX: Verificar que la posicion en pantalla es valida (headScreen no fue calculada si huesos fallaron)
            if (info.headScreen.x < 1 && info.headScreen.y < 1) continue;
