...
#include "vector.h"
#include "lib.h"
#include "MinHook/include/MinHook.h" // FIX: Usar ruta relativa en lugar de absoluta
#include "offset.h"
...
// FIX: Agregar límite y limpieza periódica al caché para evitar crecimiento indefinido
const size_t MAX_CACHE_SIZE = 10000;
static auto lastCacheCleanTime = std::chrono::steady_clock::now();

void CleanCacheIfNeeded() {
    auto now = std::chrono::steady_clock::now();
    auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - lastCacheCleanTime).count();
    
    // Limpiar caché cada 10 segundos o si excede el límite
    if (elapsed > 10 || CacheZ.size() > MAX_CACHE_SIZE) {
        std::unique_lock<std::shared_mutex> lock(zCacheSharMtx);
        
        // Eliminar entradas viejas (más de 5 segundos)
        auto it = CacheZ.begin();
        while (it != CacheZ.end()) {
            auto entryAge = std::chrono::duration_cast<std::chrono::seconds>(now - it->second.zTimeTemp).count();
            if (entryAge > 5) {
                it = CacheZ.erase(it);
            } else {
                ++it;
        ...
        // Si todavía es muy grande, eliminar las más viejas hasta llegar al límite
        while (CacheZ.size() > MAX_CACHE_SIZE) {
            auto oldest = CacheZ.begin();
            for (auto iter = CacheZ.begin(); iter != CacheZ.end(); ++iter) {
                if (iter->second.zTimeTemp < oldest->second.zTimeTemp) {
                    oldest = iter;
                }
            }
            CacheZ.erase(oldest);
...
bool ConvertZ(uintptr_t address, uintptr_t& phys) {
    // FIX: Limpiar caché periódicamente
    CleanCacheIfNeeded();
